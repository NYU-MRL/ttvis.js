...
function ttTick() {
	if (tt != undefined && tt.myState == undefined) {
		tt.waitForDomReady(document, function() {
			tt.load(function(error) {
				...
				console.log("tt initialized:");
				console.log(tt.myState);

				// init forces array
				ttForce = newZeroArray(tt.myState.hmd.forces.length);
			});
		});
	}
	...
	if (tt.myState !== undefined) {
		...
		for (var i = 0 ; i < 1024 ; i++) {
			hi = Math.max(hi, ttForce[i]);
			lo = Math.min(lo, ttForce[i]);
		}
		// console.log(lo + " " + hi);
	}
}
